...
export default class DynamicMesh {
    ...
    setisoLines() {
        ...
        for (const i in this.polygonPositons[this._index]) {
            if (i % 3 === 0) {
                let location = turf.point([this.polygonPositons[this._index][i], this.polygonPositons[this._index][Number(i) + 1]], { height: this.polygonPositons[this._index][Number(i) + 2] })
                pointarr.push(location)
            }
        }

        let grid = turf.featureCollection(pointarr)
        // console.log(grid)
        let breaks = []
        ...
        let lineprimintives = this.viewer.scene.primitives.add(lineprimitive)
        ...
        for (const j in this.polygonPositons[this._index]) {
            if (j % 3 === 0) {
                if (this.polygonPositons[this._index][Number(j) + 2] > this.maxH) {
                    ...
                    let Cindex = parseInt((this.polygonPositons[this._index][Number(j) + 2] - this.minH) / (this.maxH - this.minH) * 100)
